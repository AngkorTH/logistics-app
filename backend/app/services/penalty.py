...
def add_penalty(db: Session, trip: Trip, actor: User, amount: float, reason: str) -> Penalty:
    ...
    _notify_admin(db, trip, actor, row)
    return row


def _notify_admin(db: Session, trip: Trip, actor: User, row: Penalty) -> None:
    """แจ้ง Admin เมื่อมีการหักเงิน — **บันทึกลงกล่องจดหมายจริง (Notification ใน DB)**

    เดิมเป็นแค่ logger.info ซึ่งไม่มีใครเห็น · ตอนนี้เด้งขึ้นกระดิ่งฝั่งบริหารทันที
    """
    from app.services.notification import push_notification

    push_notification(
        db, "PENALTY_APPLIED", f"หักเงินคนขับ · {trip.code}",
        f"{who_label(actor)} หักเงิน {row.amount:.2f} บาท · เหตุผล: {row.reason} "
        f"(ยอดหักรวมของทริปนี้ {trip.penalty:.2f})",
        trip.id,
    )
    logger.info("[NOTIFY->ADMIN] %s หักเงินทริป %s %.2f บาท · %s",
                who_label(actor), trip.code, row.amount, row.reason)
