...
def test_full_lifecycle_white_orange_green_white(db_session, driver, supervisor):
    ...
    finish_loading(db_session, trip, driver, 13.7563, 100.5018, odometer_start=1000, odometer_photo_b64=ODO_PHOTO, loaded_photo_b64=PHOTO)
    ...
    record_delivery(db_session, trip.drops[0], driver, 13.81, 100.6, photo_b64=PHOTO)
    assert trip.status is TripStatus.WHITE
    assert trip.completed_at is None

    # ขาถัดไปวิ่งเองไม่ได้ — ต้องให้คนคุมงานจ่ายงานย่อยใหม่ก่อน
    with pytest.raises(TransitionError):
        record_delivery(db_session, trip.drops[1], driver, 13.82, 100.6, photo_b64=PHOTO)
    assign_trip(db_session, trip, "1กก-1234", supervisor)
...
def test_finish_loading_soft_block_tarpaulin(db_session, driver, supervisor):
    ...
    pass_inspection(db_session, trip, driver)
    finish_loading(db_session, trip, driver, 13.75, 100.5, odometer_start=1000, odometer_photo_b64=ODO_PHOTO, loaded_photo_b64=PHOTO)  # ต้องไม่ throw
    assert trip.status is TripStatus.GREEN
...
def test_finish_loading_out_of_sequence_warns(db_session, driver):
    ...
    finish_loading(db_session, trip, driver, 13.75, 100.5, force=True, odometer_start=1000, odometer_photo_b64=ODO_PHOTO, loaded_photo_b64=PHOTO)
    assert trip.status is TripStatus.GREEN
    assert trip.override is True
    assert _count_gps(db_session, GpsEvent.LOADED) == 1
...
def test_finish_loading_idempotent_double_tap(db_session, driver, supervisor):
    ...
    pass_inspection(db_session, trip, driver)
    finish_loading(db_session, trip, driver, 13.75, 100.5, odometer_start=1000, odometer_photo_b64=ODO_PHOTO, loaded_photo_b64=PHOTO)
    finish_loading(db_session, trip, driver, 13.75, 100.5, odometer_start=1000, odometer_photo_b64=ODO_PHOTO, loaded_photo_b64=PHOTO)  # กดซ้ำ
    assert _count_gps(db_session, GpsEvent.LOADED) == 1
...
def test_record_delivery_idempotent(db_session, driver, supervisor):
    ...
    record_delivery(db_session, d, driver, 13.8, 100.6, photo_b64=PHOTO)  # ซ้ำ
...
def test_close_warns_when_photos_incomplete(db_session, driver, supervisor):
    ...
    pass_inspection(db_session, trip, driver)
    finish_loading(db_session, trip, driver, 13.75, 100.5, odometer_start=1000, odometer_photo_b64=ODO_PHOTO, loaded_photo_b64=PHOTO)
    record_delivery(db_session, trip.drops[0], driver, 13.8, 100.6, photo_b64=PHOTO)  # ส่งแค่จุดเดียว
